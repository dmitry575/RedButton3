...
class CTools_zip
  {
  /*
   CZip class, for reading or writing .zip files
  ...
  function create_dir($name) // Adds a directory to the zip with the name $name
    ...
    else $name = iconv('windows-1251', 'CP866//TRANSLIT//IGNORE', $name);
  ...
  /**
   * в буфер вычитывается весь zip архив и потом распаковывается
   * @param string $filedata
   * @param bool $create_files
   * @return array
   */
  public function read_buffer($filedata, $create_files)
    {
    // Break into sections
    ...
    foreach($filesecta as $filedata)
      ...
      //---
      if($create_files) if(!file_exists($entrya['dir'])) mkdir($entrya['dir']);
      ...
      if($isencrypted)
        ...
        //--- не много по экономим памяти
        if(!empty($entrya['dir'])) $dir = trim($entrya['dir'], '\\/') . '/';
        else $dir = '';
        if($create_files) file_put_contents($dir . $entrya['name'], $filedata);
        else             $entrya['data'] = $filedata;
        }
      //--- не много по экономим памяти
      if(!$create_files) $this->files[] = $entrya;
      }
    return $this->files;
    }
   ...
   * @return array
   */
  function read_zip($name, $create_files = true)
    {
    // Clear current file
    $this->datasec = array();
    // File information
    ...
    fclose($fh);
    //---
    return $this->read_buffer($filedata, $create_files);
    }
   ...
   * @param $file
   * @param string $dir
   * @param array $file_blacklist
   * @param array $ext_blacklist
   * @return bool
   */
