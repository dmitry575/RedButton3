...
class CModel_CryptAlg
{
     ...
     * @return void
     */
    public function Init($aes_key)
    {
        $this->m_nk = strlen($aes_key) / 4;
     ...
     **/
    /**
     * @param string $x 16-byte ciphertext
     * @return string
     */
    public function encryptBlock($x)
    {
        ...
        for ($i = 0; $i < 4 * self::$m_nb; $i++) $y .= chr($this->m_cipher_table[$i % 4][($i - $i % self::$m_nb) / self::$m_nb]);
        return $y;
    }
    ...
    private function shiftRows()
    {
        $temp = array();
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$m_nb; $j++) $temp[$j] = $this->m_cipher_table[$i][($j + $i) % self::$m_nb];
            for ($j = 0; $j < self::$m_nb; $j++) $this->m_cipher_table[$i][$j] = $temp[$j];
        }
    }
