 ...
 * Парсер для картинок
 */
class CTexts extends IPage
  {
  private $m_settings;
  private $m_global_settings;
  private $m_model;
  //--- переводы
  protected static $m_translate = array('ru' => array('main_title'              => 'Парсер текстов',
                                                      'b_language'              => 'Язык',
...
'b_pause'                 => 'Кол-во секунд между запросами к поисковой системе:',
    ...
    //---
                                        'en' => array('main_title'              => 'Text Parsing',
                                                      'b_language'              => 'Language',
...
'b_pause'                 => ' sec pause between request to search',
  ...
  public function __construct()
    ...
    //---
    $this->m_model = new CModel_ParserTexts();
    //---
    $this->m_settings = new CModel_ParseSettings();
    $this->m_settings->Load("texts");
    //---
...
//---    
  ...
  public function Show($path = null)
    ...
    //---
    include("./inc/pages/texts/index.phtml");
    if(isset($_SESSION['texts_savesettings'])) unset($_SESSION['texts_savesettings']);
    }
  ...
  private function OnSaveTask($url)
    ...
    if(empty($filename)) $filename = 'text-' . uniqid() . '.txt';
    ...
    //---
    $this->m_settings->Save($_POST, 'texts');
    //---
    $this->m_model->SaveTask($_POST, CModel_ParserTexts::TYPE_TEXT);
    //---
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " task saved");
    //---
    $_SESSION['texts_savesettings'] = 1;
    //--- редиректим на страницу со списком задач
    header("location: ?module=texts");
    exit;
    }
  ...
  private function OnStopTask()
    ...
    //---
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " tasks stopped by action OnStopTask");
  ...
  private function OnClearTask()
    {
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " clear task begin");
    $this->m_model->ClearTask();
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " clear task finished");
    //--- редиректим на страницу со списком тасков
    header("location: ?module=texts");
    exit;
    }
  ...
  private function OnRunTasks()
    ...
    $path     = $urlArray['path'] . '?module=texts&' . urlencode('a[starttask]');
    //---
  ...
  private function OnStartTask()
    ...
    //---
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " task starting");
    //---
    if(empty($list))
      {
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " no task for start");
      return;
      }
    //--- старт задач
    $this->m_model->StartTask();
    ...
    foreach($list as $file_task)
      ...
      if(empty($task_info))
        {
        CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " task not found " . $file_task);
        continue;
        }
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' checking');
      //--- нужно ли запускать задачу
      if($task_info['status'] == CModel_ParserTexts::STATUS_FINISH) continue;
      //---
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' starting');
      //--- изменить статус на выполнение
      $this->m_model->UpdateStatusStarting($task_info);
      //--- записи в лог
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' start');
      //--- вычитаем ключевики
      $keywords = $this->m_model->GetKeywords($task_info['task']);
      //---
      if(empty($keywords))
        {
        CLogger::write(CLoggerType::ERROR, CModel_ParserTexts::PREFIX_LOG . ' keywords is empty ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' finished');
        //--- закончили
        $this->m_model->UpdateStatusFinish($task_info);
        CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' finished');
        continue;
        }
      //---
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' parsing starting, keywords: ' . count($keywords) . ', threads: ' . (!empty($task_info['task']['threads']) ? "many" : "one"));
      //--- запустим парсер в зависимости от многопоточности
      if(!empty($task_info['task']['threads']))
        {
        //var_dump($task_info['task']);
        //--- много поточность
      ...
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' parsing finished');
      //--- закончили
      $this->m_model->UpdateStatusFinish($task_info);
      CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . ' ' . $file_task . ' - ' . $task_info['number'] . ' - ' . date("d.m.Y H:i", $task_info['date_create']) . ' finished');
      }
    ...
    //---
    CLogger::write(CLoggerType::DEBUG, CModel_ParserTexts::PREFIX_LOG . " task finished");
    }
   ...
   * @param $url
   */
  public function OnDeleteTexts($url)
    {
    $this->m_model->GetListTask();
    $ids = explode(',', $_REQUEST['texts_ids']);
    //---
    foreach($ids as $id)
      {
      ...
      if($this->m_model->Delete($id))
        {
        CLogger::write(CLoggerType::DEBUG, "text deleted " . $id);
        }
      }
    //---
    header("location: ?module=texts");
   ...
   * @param $url
   */
  public function OnRepeatTexts($url)
    {
    $ids = explode(',', $_REQUEST['texts_ids']);
    foreach($ids as $id)
      {
      ...
      $text_info['status'] = CModel_ParserTexts::STATUS_BEGIN;
      if($this->m_model->UpdateTask($text_info)) CLogger::write(CLoggerType::DEBUG, "text update status BEGIN " . $id);
      }
    //---
    header("location: ?module=texts");
    exit;
